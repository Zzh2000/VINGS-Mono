...
import frontend_vo.geom.projective_ops as pops
from frontend_vo.modules.corr import CorrBlock, AltCorrBlock
...
from frontend_vo.modules.extractor import BasicEncoder
from frontend_vo.droid_net_nerfslam import UpdateModule


from frontend_vo.droid_net_nerfslam import cvx_upsample
...
class RaftVisualFrontend(VisualFrontend):
    def __init__(self, cfg, device="cpu"):
        ...
        world_T_body_t0 = Pose3(np.eye(4))
        body_T_cam0     = Pose3(np.eye(4))
        ...
        self.world_T_body_t0 = world_T_body_t0 # (7,)
        self.body_t0_T_world = gtsam_pose_to_torch(self.world_T_body_t0.inverse(), self.device, torch.float) # (7,)
        self.body_T_cam0 = body_T_cam0 # gtsam(7,)
        self.world_T_cam0_t0 = world_T_body_t0 * body_T_cam0 # gtsam(7,)
        self.cam0_t0_T_world = gtsam_pose_to_torch(self.world_T_cam0_t0.inverse(), self.device, torch.float)  # (7,)
        self.cam0_T_body = gtsam_pose_to_torch(body_T_cam0.inverse(), self.device, torch.float)   # (7,)

        # Frontend params
        # self.keyframe_thresh = 4.0 # Distance to consider a keyframe, threshold to create a new keyframe [m] # why not 0.4!
        # TTD 2024/11/01
        if 'keyframe_thresh' in self.cfg['frontend'].keys():
            self.keyframe_thresh = self.cfg['frontend']['keyframe_thresh']
        else:
        ...
        weights = self.load_weights(cfg['frontend']['weight'])
        ...
        # TTD 2024/08/21
        # if 'use_imu' in list(cfg['tracker'].keys()) and cfg['tracker']['use_imu']:
        #     self.imu_model = IMUModel(cfg)
    ...
    def initialize_buffers(self, image_size):
        ...
        self.cam0_T_world[:]     = self.cam0_t0_T_world # w2c
        self.world_T_body[:]     = gtsam_pose_to_torch(self.world_T_body_t0, device=self.device, dtype=torch.float) # i2w
    ...
    batch["poses"].shape: (1, 4, 4)
    batch["t_cams"].shape: (1,)
    batch["calibs"]: [camera_model class]
    '''
    #@abstractclassmethod
    def forward(self, batch):
        ...
        if self.last_k is None:
            ...
            if 'sense_depths' in list(batch.keys()) and batch["sense_depths"][0] is not None:
                # self.gt_depths.shape = (N, 1, H, W)
               self.gt_depths[self.kf_idx]   = torch.tensor(batch["sense_depths"][0],          device=self.device).permute(2,0,1)
        ...
        # (N, 4, 4)
        if 'sense_depths' in list(batch.keys()) and batch["sense_depths"][0] is not None:
            self.gt_depths[self.kf_idx]   = torch.tensor(batch["sense_depths"][0],          device=self.device).permute(2,0,1) # (1, H, W)
        ...
        # (N, 3, 344, 616)
        self.cam0_intrinsics[self.kf_idx] = (1.0 / self.dsf) * torch.tensor(batch["calibs"][0].camera_model.numpy(), device=self.device)
        # (N, 4) | [fv, fu, cv, cu] | [38.5000, 37.9412, 38.4679, 21.4684]
        ...
        # TTD 2024/08/19
        if "sense_depths" in list(batch.keys()) and batch["sense_depths"][0] is not None:
            self.cam0_idepths[self.kf_idx]          = (1./torch.tensor(batch["sense_depths"][0], device=self.device).permute(2,0,1))[:, ::8, ::8]
            self.cam0_idepths_sensed[self.kf_idx]   = (1./torch.tensor(batch["sense_depths"][0], device=self.device).permute(2,0,1))[:, ::8, ::8]
        ...
        if not self.is_initialized:
            if self.kf_idx >= self.keyframe_warmup: # 8
                self.__initialize()
            else:
        ...
        # 下面的是 not self.is_initialized 或 accept_this_kf的;
        self.last_k                       = k
        self.last_kf_idx                  = self.kf_idx
        self.kf_idx_to_f_idx[self.kf_idx] = k # not really necessary I think, me too.
        self.f_idx_to_kf_idx[k]           = self.kf_idx
    ...
    # TTD 2024/05/05
    def forward_fill(self, batch, graph_dict):
        '''
        Description:
            每一张图都要跑一次forward_fill, 但是只有在最后一次的时候才会BA,并返回优化好的Pose;
        Input:
            batch
                 ["images"]:
                 ["poses"]:
        ...
        if self.last_k is None:
            ...
            # Initialize network buffers
            self.initialize_buffers(imgs_k.shape[-2:]) # last two dims are h,w
            if "poses" in list(batch.keys()):
                self.gt_poses[self.kf_idx]        = torch.tensor(batch["poses"][0],           device=self.device)
            if batch["sense_depths"][0] is not None:
                self.gt_depths[self.kf_idx]   = torch.tensor(batch["sense_depths"][0],          device=self.device).permute(2,0,1)
            self.cam0_timestamps[self.kf_idx] = torch.tensor(batch["t_cams"][0],          device=self.device)
            self.cam0_images[self.kf_idx]     = torch.tensor(batch["images"][0],          device=self.device)[..., :3].permute(2,0,1)
            self.cam0_intrinsics[self.kf_idx] = (1.0 / self.dsf) * torch.tensor(batch["calibs"][0].camera_model.numpy(), device=self.device)
            ...
            self.f_idx_to_kf_idx[k]           = self.kf_idx
            # ----------------------------------------------
            self.cam0_T_body[self.kf_idx] = None
            self.cam0_T_world[self.kf_idx] = None
            self.world_T_body[self.kf_idx] = None
            self.cam0_idepths[self.kf_idx] = batch["idepths"]
            self.cam0_depths_cov[self.kf_idx] = None
            self.cam0_idepths_cov[self.kf_idx] = batch["idepths_cov"]
            self.gru_estimated_flow[self.kf_idx] = None # 渲染赋值
            self.gru_estimated_flow_inactive[self.kf_idx] = None # 渲染赋值
            # ----------------------------------------------
            self.kf_idx += 1
            # ----------------------------------------------
            
            
            if self.kf_idx >= self.keyframe_warmup-1: # 15
                # self.__initialize()
                '''
                (1) __initialize方法干了三件事儿: 
                        - (1) add_factors;
                        - (2) kf0.1=None然后 update 了八次;
                        - (3) 把第九帧的self....赋值成第八帧滴;
                (2) update之前一定要准备好:          
                        - (1) self.ii, self.jj, self.cam0_T_body, self.gru_estimated_flow               | manual assign
                            self.cam0_idepths, self.cam0_depths_cov                                   |
                            self.gru_estimated_flow_inactive, self.gru_estimated_flow_weight_inactive |
                            self.ii_inactive, self.jj_inactive                                        |
                        - (2) self.correlation_volumes                                                  | add_factors
                        - (3) self.gru_hidden_states, self.gru_contexts_input                           | 给None就行?
                
                计算graph_dict['ii'],graph_dict['jj']还是挺难的, 感觉需要比较复杂的计算;
                '''
                ii, jj = graph_dict['ii'], graph_dict['jj']
    ...
    def update(self, kf0=None, kf1=None, itrs=2, use_inactive=False, EP=1e-7, motion_only=False):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            self.gru_estimated_flow        = coords1 + flow_delta.to(dtype=torch.float) # (1, 42, 43, 77, 2)
            ...
            # ic("BA!")
            '''
            gru_estimated_flow: torch.Size([42, 2, 43, 77])
            gru_estimated_flow_weight: torch.Size([42, 2, 43, 77])
            damping: torch.Size([9, 43, 77])
            ii: torch.Size([42])
            jj: torch.Size([42])
            kf0: 0
            kf1: None
            ...
            self.cam0_depths_cov_up[kx] = cvx_upsample(self.cam0_depths_cov[kx].unsqueeze(-1), upmask).squeeze()

            
            '''
            Here are 2 type of flow.
            (1) gru_flow;
            (2) reprojection_flow;
            '''
    ...
    def update_lowmem(self, t0=None, t1=None, itrs=2, use_inactive=False, EP=1e-7, steps=8):
        ...
        for step in range(steps):
            ...
            for i in range(0, self.jj.max() + 1, s): # what does this do?
                ...
                self.cam0_depths_cov_up[all_kf_ids] = cvx_upsample(self.cam0_depths_cov[all_kf_ids].unsqueeze(-1), upmask).squeeze()
    ...
    def __update(self):
        ...
        #ic(d.item())
        if d.item() < self.keyframe_thresh: # 4.0
            #ic("Not a keyframe")
            return False
        else:
            #ic("Found a keyframe")
    ...
    def __initialize(self):
        ...
        self.add_proximity_factors(kf0=0, kf1=0, rad=2, nms=2, thresh=self.frontend_thresh, remove=False)
        ...
        '''
            self.cam0_T_world.shape: torch.Size([100, 7])
            self.world_T_body.shape: torch.Size([100, 7])
        self.world_T_body_cov.shape: torch.Size([100, 6, 6])
            self.cam0_idepths.shape: torch.Size([100, 43, 77])
        self.cam0_idepths_cov.shape: torch.Size([100, 43, 77])
         self.cam0_depths_cov.shape: torch.Size([100, 43, 77])
         -------------------------------------------------------
         self.gru_estimated_flow.shape: torch.Size([1, 48, 43, 77, 2])
          self.gru_hidden_states.shape: torch.Size([1, 48, 128, 43, 77])
         self.gru_contexts_input.shape: torch.Size([1, 48, 128, 43, 77])
         -------------------------------------------------------
                self.gru_estimated_flow_inactive.shape: torch.Size([1, 0, 43, 77, 2])
         self.gru_estimated_flow_weight_inactive.shape: torch.Size([1, 0, 43, 77, 2])
        '''
    ...
    def add_proximity_factors(self, kf0=0, kf1=0, rad=2, nms=2, beta=0.25, thresh=16.0, remove=False):
        ...
        for i, j in zip(ii1.cpu().numpy(), jj1.cpu().numpy()):
            for di in range(-nms, nms+1):
                for dj in range(-nms, nms+1):
                    # max(min(abs(i-j)-2, nms), 0) ∈ {0, 1, 2}
                    if abs(di) + abs(dj) <= max(min(abs(i-j)-2, nms), 0):
                        i1 = i + di
                        j1 = j + dj

                        if (kf0 <= i1 < t) and (kf1 <= j1 < t):
                            # 把 (ii-jj).sum() < 2的设置成inf;
        ...
        for i in range(kf0, t):
            ...
            for j in range(max(i-rad-1,0), i):
                es.append((i,j))
                es.append((j,i))
                d[(i-kf0)*(t-kf1) + (j-kf1)] = np.inf # 这是干啥

        ix = torch.argsort(d)
        # 添加非邻居边直到加满， 按照 distance(ii, jj)从小到大的顺序；
        for k in ix:
            if d[k].item() > thresh: # 16
                continue

            if len(es) > self.max_factors: # 48
                break
    ...
    def distance(self, ii=None, jj=None, beta=0.3, bidirectional=True):
        ...
        if bidirectional:
            ...
            d1 = droid_backends_nerfslam.frame_distance(poses, self.cam0_idepths, self.cam0_intrinsics[0], ii, jj, beta)
            d2 = droid_backends_nerfslam.frame_distance(poses, self.cam0_idepths, self.cam0_intrinsics[0], jj, ii, beta)
            d = .5 * (d1 + d2)
        else:
            d = droid_backends_nerfslam.frame_distance(self.cam0_T_world, self.cam0_idepths, self.cam0_intrinsics[0], ii, jj, beta)
    ...
    def ba(self, gru_estimated_flow, gru_estimated_flow_weight, damping, ii, jj, 
        ...
        # ii,jj are indes for keyframelist.
        if kf1 is None:
        ...
        kf_ids = [i+kf0 for i in range(kf1 - kf0)] # 这是干啥, range(kf0, kf1)
        ...
        for _ in range(itrs):
            ...
            '''
            self.cam0_T_world: (100, 7)
            self.world_T_body: (100, 7)
            self.cam0_idepths: (100, 43, 77)
            self.cam0_intrinsics[0]: tensor([38.5000, 37.9412, 38.4679, 21.4684], device='cuda:0')
            self.cam0_T_body: tensor([-0., -0., -0., 0., 0., 0., 1.], device='cuda:0')
            self.cam0_idepths_sensed: (100, 43, 77)
            gru_estimated_flow: (42, 2, 43, 77)
            gru_estimated_flow_weight: (42, 2, 43, 77)
            damping: (9, 43, 77)
            '''
            ...
            # TTDF 2024/04/21
            xi_delta = torch.as_tensor(gtsam_delta.vector(), device=self.device, dtype=torch.float).view(-1, 6)
            
            
            droid_backends_nerfslam.solve_depth(xi_delta, self.cam0_idepths, Q, E, w, ii, jj, kf0, kf1)
            self.cam0_idepths.clamp_(min=0.001)
        
        # 🤡Compute Uncertainty.
        '''
        Let's explore what are H, v, Q, E, w.
        H.shape = (42, 42) → L.shape = (42, 42)
        v.shape = (42)
        Q.shape = (10, 43*77)
        E.shape = (57, 6, 43*77)
        w.shape = (10, 43*77)
        ii, jj: (50), (50), 这个应该是针对global_kf的索引
        kf0=14, kf1=21, 这俩应该也是针对global kf的索引
        pose_keys: length=7
        '''
        if compute_covariances:
            ...
            if L is not None:
                ...
                #ic(K)
                #ic(P)
                # 为了 "[ii-min(ii,jj)] < unique(ii).shape[0]" 恒成立呢
                ...
                # depth_cov = z_cov  / self.cam0_idepths[kx]**1
    ...
    def get_viz_out(self, batch):
        ...
        if len(viz_index) != 0:
            ...
            '''
            INFO 2024/05/17
            (1) viz_index是对visual_frontend.gt_poses, cam0_images...的索引;
                self.cam0_timestamps[self.kf_idx] = batch["t_cams"][0]
                self.cam0_timestamps记录的是 kf_id → f_id 的索引;
                "现在急需的是 viz_out_id → kf_id 的索引"
                get_vizout每次返回的是一个viz_out字典, 这个字典的索引怎么拿到

            (2) viz_index, = torch.where(self.viz_idx)
                self.viz_idx[kf0:self.kf_idx+1] = True
            (3) batch["t_cams"]是合理的
            emmm目前感觉的是 kf_idx_to_f_idx 和 self.cam0_timestamps 有点区别;
            '''
            # ic(out_device)
            # torch.save({'rgb': images.cpu(), 'viz_index': viz_index.cpu(), 'kf_idx_to_f_idx': self.kf_idx_to_f_idx}, './debug.pt')
    ...
    def __initialize_gt_pose(self):
        '''
        emmm这里要注意把位姿变换成可以用的哈
        '''
        import os
        gt_c2w_folder = os.path.join(self.cfg['dataset']['folder'], "pose")
        # curr_f_idx = [self.kf_idx_to_f_idx[idx] for idx in range(9)]
        for idx in range(9):
            curr_f_idx = self.kf_idx_to_f_idx[idx]
            gt_c2w = Pose3(np.loadtxt(os.path.join(gt_c2w_folder, '{}.txt'.format(curr_f_idx))))
            
            if idx == 0:
               self.cam0_T_world[:] = gtsam_pose_to_torch(gt_c2w, self.device, torch.float) # (7,)
               self.world_T_body[:] = gtsam_pose_to_torch(gt_c2w.inverse(), self.device, torch.float) # (7,)
               
               self.body_T_cam0 = gtsam_pose_to_torch(gt_c2w.inverse(), self.device, torch.float) # (7,)
               self.cam0_t0_T_world = gtsam_pose_to_torch(gt_c2w, self.device, torch.float) # (7,)
                
            else:
                self.cam0_T_world[idx] = gtsam_pose_to_torch(gt_c2w, self.device, torch.float) # (7,)
                self.world_T_body[idx] = gtsam_pose_to_torch(gt_c2w.inverse(), self.device, torch.float) # (7,)

            # gtsam_pose_to_torch(Pose3())
