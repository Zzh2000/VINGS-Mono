    'square-2':       [292, 1073, 107, 453, 537, 30, 471, 1046, 17, 277, 1057, 541, 9, 70, 71, 296, 46, 254, 857, 1195, 813, 735, 495, 523, 1040, 691, 61, 828, 1013, 962, 298, 253, 208, 1055, 808, 37, 190, 427, 990, 505, 1178, 841, 526, 18, 1038, 1165, 1086, 4, 1180, 934],
}
...
def render_and_eval_sequence(seq_path, output_folder, use_full_traj_for_poses=True, n_eval=50):
    ...
    # Locate run directory (most recent with a non-empty ply/ sub-folder)
    run_dir = None
    for d in [seq_output] + list(reversed(subdirs)):
    ...
    # Locate trajectory — search most-recent subdir first
    traj_file = None
    ...
    # Override with full-resolution dataset intrinsics (no distortion)
    ...
    if seq_name in UTMM_EVAL_INDICES:
        eval_indices = [i for i in UTMM_EVAL_INDICES[seq_name] if i < n_frames]
    ...
    for save_i, idx in enumerate(eval_indices):
        ...
        # Load GT at original full resolution (avoids double-resize artifact)
        import cv2 as _cv2
        rgb_path = dataset.rgbinfo_dict['filepath'][idx]
        rgb_raw  = _cv2.imread(rgb_path)                          # (H_orig, W_orig, 3) BGR
        gt = torch.tensor(rgb_raw[..., [2, 1, 0]], dtype=torch.float32
                          ).permute(2, 0, 1).to(device) / 255.0  # (3, H, W) RGB
...
def run_sequence(seq_path, config_template_path, output_folder,
    ...
    # Prefer traj_full.txt (per-frame, from PoseTrajectoryFiller) when requested
    traj_file = None
    if use_full_traj:
    ...
    # Fall back to combining keyframe poses from droid_c2w
    if traj_file is None:
        for search_dir in [seq_output] + subdirs:
...
def main():
    ...
    if 'euler' in hostname or hostname.startswith('eu-'):
        dataset_base = '/cluster/project/cvg/zihzhu/Datasets/UTMM_Dataset'
    else:
        dataset_base = '/home/zihzhu/data/Datasets/UTMM_Dataset'

    config_template = os.path.join(
        os.path.dirname(os.path.abspath(__file__)),
        '../configs/utmm/utmm.yaml'
    )
    output_folder = os.path.join(
        os.path.dirname(os.path.abspath(__file__)),
        '../output_noLoop_noMetric_MappingNoMask_fulltraj/utmm_eval'
    )

    parser = argparse.ArgumentParser(description='Run VINGS-Mono on UTMM dataset')
    ...
    parser.add_argument('--seqs',      type=str, nargs='+', default=None,
                        help='Specific sequences, e.g. ego-centric-1 square-1')
    ...
    for seq in seqs:
        if not os.path.isdir(seq):
            ...
            continue
        if 'slow-straight-1' in seq or 'slow-straight-2' in seq:
            args.config = '/cluster/project/cvg/zihzhu/VINGS-Mono/configs/utmm/utmm_moreKF.yaml'
        print(f'\n{"="*60}')
    ...
    if psnr_values:
        ...
        print('SSIM & ' + ' & '.join(f'{v:.3f}' for v in ssim_values) + f' & {np.mean(ssim_values):.3f} \\\\')
        print('LPIPS & ' + ' & '.join(f'{v:.3f}' for v in lpips_values) + f' & {np.mean(lpips_values):.3f} \\\\')
